  ...
  v-row(no-gutters)
    v-col(cols=12)
      v-sheet.pa-5(min-height='calc(100vh - 100px)' rounded='lg')
        ...
        v-row(justify='end')
          v-col(cols='auto')
            v-btn(@click='dialog = true' color='orange' text)
              | Adicionar
        v-row
          v-col.pa-0(cols='12')
            v-toolbar.pa-0(flat)
      ...
      v-card
        ...
        v-card-text
          v-container
            v-row
              v-col(cols='12')
                v-text-field(v-model='dialogData.cpf' label='Agendamento (CPF)*' required)
              v-col(cols='12')
                v-text-field(v-model='dialogData.name' label='Nome do paciente*' required)
              v-col(cols='12')
                v-autocomplete(v-model='dialogData.dentist' :items="dentists" label='Dentista responsável*' required)
              v-col(cols='12')
                v-menu(v-model='menu' :close-on-content-click='false' :nudge-right='40' transition='scale-transition' origin="bottom left" top offset-y min-width='auto')
                  ...
                  v-date-picker(
                    ...
                    locale='pt-BR')
              v-col(cols='12')
                v-select(v-model='dialogData.time' :items='schedules' label='Horário*' required)
          small *indica os campos obrigatórios
        v-card-actions
          v-btn(color='error darken-1' text @click='dialog = false')
            | Cancelar
          v-spacer
          v-btn(color='primary darken-1' text @click='addEvent')
            | Criar
</template>
